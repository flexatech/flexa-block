...
/**
 * Product Name block — server-side CSS generator.
 *
 * Produces responsive, dark-mode-aware CSS for one product-name instance.
 ...
 * `.flexa-product-name-<id>`; typography, colour and effects target the inner
 * `.flexa-product-name-<id> .flexa-product-name__title`.
 *
 * @package Flexa\Block
 */
...
/**
 * Product Name CSS generator.
 */
class Product_Name_CSS {
	...
	/**
	 * Generate CSS for a product-name instance.
	 *
	...
	public static function generate( $attrs, $css ) {
		...
		$wrap    = '.flexa-product-name-' . $id;
		$content = $wrap . ' .flexa-product-name__title';
		$hover   = $content . ':hover';
		...
		foreach ( self::$devices as $device ) {
			...
			// Typography on the title element.
		...
		// Text colour / gradient (light + dark) on the title.
