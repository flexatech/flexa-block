<?php
/**
 * Grid block — server-side render.
 *
 * CSS is generated at save time by Grid_CSS and printed inline on the front end.
...
$classes = [
	'flexa-grid',
	'flexa-grid--' . sanitize_html_class( $container_type ),
];

if ( '' !== $block_id ) {
	$classes[] = 'flexa-grid-' . sanitize_html_class( $block_id );
}
...
if ( $is_boxed ) {
	printf(
		'<%1$s %2$s%3$s><div class="flexa-grid__inner"%4$s>%5$s</div></%1$s>',
		esc_html( $html_tag ),
