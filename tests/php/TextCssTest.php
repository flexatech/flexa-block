<?php
/**
 * Tests for the Text block CSS generator.
 *
 * @package Flexa\Block
 */

use Flexa\Block\CSS_Generators\Text_CSS;

/**
 * @covers \Flexa\Block\CSS_Generators\Text_CSS
 */
class TextCssTest extends CssTestCase {

	private const WRAP    = '.flexa-text-a';
	private const CONTENT = '.flexa-text-a .flexa-text__content';
	private const HOVER   = '.flexa-text-a .flexa-text__content:hover';

	/**
	 * Convenience wrapper around the Text generator.
	 *
	 * @param array $attrs Text attributes.
	 * @return string
	 */
	private function gen( array $attrs ): string {
		return $this->genCss( [ Text_CSS::class, 'generate' ], $attrs );
	}
	...
	public function test_untouched_text_emits_nothing(): void {
		// Only a blockId: the theme should style everything, so no declarations.
		$this->assertSame( '', $this->gen( [ 'blockId' => 'a', 'htmlTag' => 'p', 'textType' => 'color' ] ) );
	}

	public function test_alignment_on_wrapper(): void {
		$css = $this->gen( [ 'blockId' => 'a', 'alignment' => [ 'desktop' => 'justify' ] ] );
		$this->assertCssHas( $css, self::WRAP, 'text-align:justify' );
	}
	...
	public function test_typography_on_content(): void {
		$css = $this->gen( [
			'blockId'    => 'a',
			'typography' => [ 'desktop' => [ 'fontSize' => [ 'value' => '20', 'unit' => 'px' ], 'fontWeight' => '600', 'lineHeight' => '1.7' ] ],
		] );
		$this->assertCssHas( $css, self::CONTENT, 'font-size:20px' );
		$this->assertCssHas( $css, self::CONTENT, 'font-weight:600' );
		$this->assertCssHas( $css, self::CONTENT, 'line-height:1.7' );
	}

	public function test_typography_tablet_in_media_query(): void {
		$css = $this->gen( [
			'blockId'    => 'a',
			'typography' => [ 'tablet' => [ 'fontSize' => [ 'value' => '16', 'unit' => 'px' ] ] ],
		] );
		$this->assertCssHasInMedia( $css, '@media (max-width: 1024px)', self::CONTENT, 'font-size:16px' );
	}

	public function test_legacy_columns_attributes_are_ignored(): void {
		// Content saved while the removed multi-column option existed still carries
		// `columns`/`columnsGap`; the generator must ignore them (no column CSS).
		$css = $this->gen( [
			'blockId'    => 'a',
			'columns'    => [ 'desktop' => '3', 'mobile' => '2' ],
			'columnsGap' => [ 'desktop' => [ 'value' => '24', 'unit' => 'px' ] ],
		] );
	...
	public function test_blend_mode_on_content(): void {
